...
def process_transcript(audio_id, transcript_segments, classifier_results):
    """
    Process transcript segments using Stay At Home Mum Langflow chain.
    ...
    for i, segment in enumerate(transcript_segments):
        ...
        # Call Langflow Stay At Home Mum chain
        try:
            result = call_langflow_chain("stay_at_home_mum_chain", segment_input)
            logger.info(f"Stay At Home Mum evaluation for segment {segment_id}: {result}")
        except Exception as e:
            logger.error(f"Error calling Stay At Home Mum Langflow chain for segment {segment_id}: {e}")
        ...
        feedback.append({
            ...
            "stay_at_home_mum": result
        })
        
        # Store individual segment feedback in Redis
        redis_conn.set(
            f"persona_feedback:stay_at_home_mum:{audio_id}:{segment_id}",
            json.dumps(result),
            ex=86400
        )
    ...
    redis_conn.set(
        f"persona_feedback:stay_at_home_mum:{audio_id}",
        json.dumps(feedback),
        ex=86400
    )
