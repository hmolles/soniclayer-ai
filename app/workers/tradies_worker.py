...
def process_transcript(audio_id, transcript_segments, classifier_results):
    """
    Process transcript segments using Tradies Langflow chain.
    ...
    for i, segment in enumerate(transcript_segments):
        ...
        # Call Langflow Tradies chain
        try:
            result = call_langflow_chain("tradies_chain", segment_input)
            logger.info(f"Tradies evaluation for segment {segment_id}: {result}")
        except Exception as e:
            logger.error(f"Error calling Tradies Langflow chain for segment {segment_id}: {e}")
        ...
        redis_conn.set(
            f"persona_feedback:tradies:{audio_id}:{segment_id}",
            json.dumps(result),
            ex=86400
        )
    ...
    redis_conn.set(
        f"persona_feedback:tradies:{audio_id}",
        json.dumps(feedback),
        ex=86400
    )
