...
def create_file_sidebar():
    ...
    if not audio_files:
        ...
        for audio in audio_files:
            ...
            id={"type": "file-item", "audio_id": audio["audio_id"]},
            n_clicks=0,
            style={
                "padding": "12px",
                "marginBottom": "4px",
...
def load_audio_file(n_clicks_list, current_audio_id):
    ...
    if ctx.triggered and ctx.triggered[0]['prop_id'] != '.':
        # Extract audio_id from the clicked item
        triggered_id = ctx.triggered[0]['prop_id']
        if 'file-item' in triggered_id:
            import json as json_module
            id_dict = json_module.loads(triggered_id.split('.')[0])
            audio_id = id_dict['audio_id']
            print(f"User selected audio: {audio_id}")
    
    # If no file clicked, use current or default
    if not audio_id:
        audio_id = current_audio_id if current_audio_id else default_audio_id
